...
exports.post = function(req, res) {
  ...
  var authorize = function(done) {
    if (status.posterId === status.recipientId) {
      return async.nextTick(function() {
        done(null, true);
      });
    }
    Friendship.findByUserIds(posterId, recipientId, function(err, friendships) {
  ...
  authorize(function(err, authorized) {
    if (err) return onErr(err, res);
    if (!authorized) return onErr('Unauthorized status', res);
